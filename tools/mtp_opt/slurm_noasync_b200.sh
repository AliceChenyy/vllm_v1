#!/bin/bash
# =============================================================================
# Slurm job: No-async-scheduling impact bench — Qwen3.5-35B-A3B on B200
# Tests O4+O5 savings under async ON vs async OFF (customer uses --no-async)
# =============================================================================

#SBATCH --job-name=noasync_bench
#SBATCH --nodes=1
...
RESULTS_DIR="/workspace/results/noasync_${SLURM_JOB_ID}_${TIMESTAMP}"

mkdir -p "${WORKSPACE_HOST}/logs" "${WORKSPACE_HOST}/results"

echo "========================================================"
echo " No-Async-Scheduling Impact Bench — B200"
